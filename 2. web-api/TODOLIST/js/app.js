...
function insertTodoData() {
    ...
    // todos 배열에 새로운 할 일을 객체로 포장해서 추가해야한다.
...
function toggleModifyMode($modifyBtn) {
    ...
    // $modifyInput.value = $oldTag.textContent;
    // const $oldTag = e.target.closest('.todo-list-item').querySelector('.text');
    // const $newTag = document.createElement('input');
    // $newTag.classList.add('modify-input');
    // $newTag.value = $oldTag.textContent;
    // e.target.closest('.todo-list-item').querySelector('.checkbox').replaceChild($newTag, $oldTag);
    // const $textSpan = $label.lastElementChild;
    // const $modifyInput = document.createElement('input');
    // $modifyInput.classList.add('modify-input');
    // $modifyInput.setAttribute('type', 'text'); // 기본값 text라 여기서는 굳이 안해줘도 됨 참고!
    // // span.text의 텍스트를 input.modify-input에 저장
    // $modifyInput.value = $textSpan.textContent;
    // $label.replaceChild($modifyInput, $textSpan)
